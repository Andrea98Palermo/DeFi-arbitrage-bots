...
//const Alglib = require('./Alglib.js-main/Alglib-v1.1.0.js');
    ...
    .on('data', async block => {
        ...
        for (let i = 0; i < fromTokens.length; i++) {
            for (let j = 0; j < toTokens.length; j++) {
                ...
                const pairAddress_2 = await bakeryFactory.methods.getPair(fromToken[i], toToken[j]).call();
                const bakeryPairContract = new web3.eth.Contract(
                ...
                const pancakeReserves = await pancakePairContract.methods.getReserves().call();
                const bakeryReserves = await bakeryPairContract.methods.getReserves().call();

                if(tokenIn < tokenOut) {
                    var m = await new BigNumber(pancakeReserves[0]).shiftedBy(-18).toNumber();
                    var n = await new BigNumber(pancakeReserves[1]).shiftedBy(-18).toNumber();
                    var x = await new BigNumber(bakeryReserves[1]).shiftedBy(-18).toNumber();
                    var y = await new BigNumber(bakeryReserves[0]).shiftedBy(-18).toNumber();
                }
                else {
                    var m = await new BigNumber(pancakeReserves[1]).shiftedBy(-18).toNumber();
                    var n = await new BigNumber(pancakeReserves[0]).shiftedBy(-18).toNumber();
                    var x = await new BigNumber(bakeryReserves[0]).shiftedBy(-18).toNumber();
                    var y = await new BigNumber(bakeryReserves[1]).shiftedBy(-18).toNumber();
                }
                /*
                console.log(`m: ${m}`);
                console.log(`n: ${n}`);
                console.log(`x: ${x}`);
                console.log(`y: ${y}`);*/
                var F_profit = function(p) {
                    //if (p >= 0.05 && p <= m && p <= x) { return (p[0]*n*y*CAKEFEE*BAKEFEE / (p[0]*n*CAKEFEE + p[0]*x + m*x)) - p[0] }
                    if (p >= 0.05 && p <= m && p <= y) { return (p[0]*n*y*CAKEFEE*BAKEFEE / (p[0]*n*CAKEFEE*BAKEFEE + p[0]*x*CAKEFEE + m*x)) - p[0] }
                    else {return - Number.MAX_VALUE}
                };
                var F_profit_neg = function (p) {return - F_profit(p)};
                /** 
                var bound_1 = function(p) {return 0.1 - p[0]};  //p >= 0.1
                var bound_2 = function(p) {return p[0] - 1000}; //p <= 1000
                */
                var opt1 = fmin.nelderMead(F_profit_neg, [0.05])['x'][0];
                var opt2 = fmin.conjugateGradient(F_profit_neg, [0.05])['x'][0];
                var opt_p = F_profit([opt1]) >= F_profit([opt2]) ? opt1 : opt2;
                //console.log(`opt_p: ${opt_p}`);
                //console.log(`Profit(opt_p): ${F_profit([opt_p])}`);
                /*
                top_p = F_profit([(Math.sqrt(m*n*x*y) - m*x) / (x + n)]) <= F_profit([-(Math.sqrt(m*n*x*y) + m*x)/(x+n)]) ? 
                            [(Math.sqrt(m*n*x*y) - m*x) / (x + n)] 
                            : [-(Math.sqrt(m*n*x*y) + m*x)/(x+n)];
                
                console.log(top_p);
                */
                maximizedProfit = await new BigNumber(F_profit([opt_p])).shiftedBy(18);
                //maximizedProfit = await new BigNumber(maximizedProfit).minus(new BigNumber(opt_p).shiftedBy(18))
                /*
                let solver = Alglib();
	            solver.add_function(F_profit);
                solver.add_less_than_or_equal_to_constraint(bound_1);
                solver.add_less_than_or_equal_to_constraint(bound_2);
                var p_max;
                solver.promise.then(function(result) { 
                    var p_guess = [0.5]; //Guess the initial values of the solution.
                    var s = solver.solve({min_max:"max", x_guess:p_guess, diffstep:0.1}); 
                    p_max = solver.get_results();
                    //    console.log("x="+x)
                    console.log(solver.get_report())
                    solver.remove() //required to free the memory in C++
                })
                
                var maximizedProfit = await new BigNumber(F_profit([p_max])).shiftedBy(18);
                */
                ...
                */
                ...
                //console.log(opt_p);
                //console.log(maximizedProfit);
                
                //if (profit > 0) {
                if (maximizedProfit > 0) {

                    console.log(`Profit without counting gas: ${maximizedProfit}`);
                    ...
                    */
                    ...
                    /*
                    const tx1 = fromContracts[i].methods.approve(
                        amount0,
                        addresses.pancake.router
                    )*/
                    ...
                    */
                    ...
                    /** 
                    const tx2 = pancakeRouter.methods.swapExactTokensForTokens

                    const [gasPrice1, gasCost1] = await Promise.all([
                        web3.eth.getGasPrice(),
                        tx1.estimateGas({from: admin}),
                    ]);

                    const tx1Cost = web3.utils.toBN(gasCost1) * web3.utils.toBN(gasPrice1);
                    profit = await new BigNumber(profit).minus(tx1Cost);
+                    */
                    ...
                    if (maximizedProfit > 0) {
                        ...
                        console.log(`
                            ...
                            Expected profit: ${maximizedProfit}
                        `);

                        /*
                        let amountReceived = await bakeryRouter.methods.swapExactTokensForTokens(
                            amountToken,
                            amountRequired,
                            path,
                            msg.sender,
